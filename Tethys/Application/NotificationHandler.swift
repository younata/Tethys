...
public struct LocalNotificationHandler: NotificationHandler {
    ...
    private func articleFromUserInfo(_ userInfo: [String: Any]) -> Future<Result<Article, TethysError>> {
        ...
        return self.feedRepository.feeds().map { result -> Result<Article, TethysError> in
            switch result {
