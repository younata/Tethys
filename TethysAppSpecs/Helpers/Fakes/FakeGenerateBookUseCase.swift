...
final class FakeGenerateBookUseCase: GenerateBookUseCase {
    ...
    var generateBookStub: ((String, String, [Article], Book.Format) -> Future<Result<URL, TethysError>>)?
    func generateBookReturns(_ returnValue: Future<Result<URL, TethysError>>) {
    ...
    func generateBook(title: String, author: String, chapters: [Article], format: Book.Format) -> Future<Result<URL, TethysError>> {
